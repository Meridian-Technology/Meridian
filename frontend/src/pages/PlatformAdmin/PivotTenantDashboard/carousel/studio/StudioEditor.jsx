...
export default function StudioEditor({ issue, onSave, onEditSelection, account, onBack }) {
  ...
  const photoInputRef = useRef(null); const photoTargetRef = useRef(null);
  ...
  const upload = async (file, apply) => {
    const check = commands.validateUpload(file); if (!check.ok) { setNotice(check.error); return; }
    ...
    try {
      ...
      const asset = result.data.data.asset; setAccountAssets(current => [asset, ...current]); apply(asset); setNotice('Image uploaded. Save to keep it in this issue.');
    } catch (error) { setNotice(error.message || 'Image upload failed. Try again.'); }
  ...
  return <div className="jg-editor pivot-ops" ref={rootRef} tabIndex={0} onKeyDown={keyDown} onPointerMove={move} onPointerUp={finish} onPointerCancel={cancelGesture} onLostPointerCapture={() => { if (drag.current) cancelGesture(); }}>
    ...
    <input ref={photoInputRef} type="file" hidden aria-label="Add photograph file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={event => { const file = event.target.files?.[0]; event.target.value = ''; const target = photoTargetRef.current; if (file && target) upload(file, asset => commit(commands.replaceImageAsset(docRef.current, target.slideId, target.id, asset), 'Add photograph')); }} />
    <div className="jg-editor__toolbar">
      <div><Button icon="lucide:undo-2" aria-label="Undo" onClick={() => setHistory(undo)} disabled={!history.past.length} /><Button icon="lucide:redo-2" aria-label="Redo" onClick={() => setHistory(redo)} disabled={!history.future.length} /><span className="jg-editor__separator" /><Button icon="lucide:type" disabled={!slide} onClick={() => { const next = commands.insertText(doc, slide.id); commit(next, 'Add text'); setSelected([next.slides[index].elements.at(-1).id]); setPanel('design'); }}>Text</Button><Button icon="lucide:image" disabled={!slide} onClick={() => setPanel('assets')}>Image</Button><Button icon="lucide:layout-template" onClick={() => setPanel('templates')}>Templates</Button></div>
    ...
    <footer className="jg-editor__footer"><span>{notice || (mode === 'crop' || mode === 'background-crop' ? 'Crop mode · Drag to pan · Escape to finish' : 'Drag to move · Shift-click to select more · Alt to bypass snapping')}</span><span>1080 × 1350 · 4:5</span></footer>
