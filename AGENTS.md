...
- **Events-Backend (CRITICAL)**: The `backend/events/` directory contains the Events-Backend module, which is **essential** for the server to start and operate correctly. It is a separate private GitHub repo (`git@github.com:Study-Compass/Events-Backend.git`), not a submodule. In the repo, `backend/events` is tracked as a symlink pointing to `../../Events-Backend` (for developers who have both repos side by side). The production build system clones it via `bin/fetch_private_deps` using the SHA pinned in `private-deps.lock`.
  - **To set up properly**: Clone the Events-Backend repo so the symlink resolves: `git clone git@github.com:Study-Compass/Events-Backend.git ../../Events-Backend` (requires SSH access to the private repo). Alternatively, run `bin/fetch_private_deps` which clones directly into `backend/events/`.
