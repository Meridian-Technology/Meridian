...
**Option A worker:** `server.js` starts `startMeridianJobWorkerLoop()` after listen unless `NODE_ENV=test` or `DISABLE_MERIDIAN_JOB_WORKER=true`. Poll interval `MERIDIAN_JOB_POLL_MS` (default `30000`). A future Heroku worker dyno should `require('./services/meridianJobWorkerLoop').startMeridianJobWorkerLoop()` — same module, no second handler registry.

**Terminal failure:** after the last retry, platform admins get email plus Expo push on ops tenant **`sf`** (`MERIDIAN_OPS_TENANT_KEY`, default `sf`). Pushes go through `expoPushDeliveryService`. Dedupes with `failureAlertSentAt`.
