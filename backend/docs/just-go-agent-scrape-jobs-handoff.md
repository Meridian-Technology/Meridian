...
- [`Meridian-Mintlify/strategy/meridian-job-notification-platform-plan.mdx`](../../Meridian-Mintlify/strategy/meridian-job-notification-platform-plan.mdx) — Meridian-executed notification jobs vs Relay compute
...
## Meridian notification jobs vs Relay compute schedules

Two operator surfaces share the **Notifications** panel. They are not the same worker.

| Work | Who runs it | Where operators look |
|---|---|---|
| City-source discovery and city-curation-refresh | **Relay** (Mini worker + schedules). Production apply/auto-apply stay on Meridian. | City **Compute jobs** (`?page=10`) and a **read-only** Compute jobs table on fleet Notifications |
| Weekly drop, ritual crew scan, solo swipe reminder, definition-driven pushes | **Meridian API** (Option A: same Node process as Express) | Fleet **Notifications** (`/platform-admin/pivot?page=5`); city **Notifications** (`/platform-admin/pivot/:tenantKey?page=9`) |

Do **not** enqueue notification sends on Relay or the Mini. Relay `scheduleId` / occurrence metadata is not shown in the compute adapter in v1. Apply/retry for compute remains on the Compute jobs tab.

**Notifications panel:** fleet failed-run queue, definition editor (30-minute cron, copy keys, tenant overrides), manual enqueue, and compute read-only inspect links. City Notifications absorbs weekly drop send/dry-run; run detail shows attempts and per-user `meridian_job_deliveries` (no push tokens). Legacy `PivotDropPushRun` history still lists for unmatched older sends.

**Option A worker:** `server.js` starts `startMeridianJobWorkerLoop()` after listen unless `NODE_ENV=test` or `DISABLE_MERIDIAN_JOB_WORKER=true`. Poll interval `MERIDIAN_JOB_POLL_MS` (default `30000`). Each tick ensures global job indexes, reclaims `running` claims older than `MERIDIAN_JOB_LEASE_MS` (default 15 minutes), and drains up to `MERIDIAN_JOB_MAX_PER_TICK` due jobs (default 8) one at a time. `SIGTERM` / `SIGINT` stop the poll loop; in-flight work is finished by the lease reclaim on the next boot. A future Heroku worker dyno should `require('./services/meridianJobWorkerLoop').startMeridianJobWorkerLoop()` — same module, no second handler registry. Run `node migrations/ensureMeridianJobIndexes.js` before the first boot if you want the unique `runKey` index in place before the process accepts traffic; the worker also creates it on the first tick.

**Terminal failure:** after the last retry, platform admins get email plus Expo push on ops tenant **`sf`** (`MERIDIAN_OPS_TENANT_KEY`, default `sf`). Pushes go through `expoPushDeliveryService`. `failureAlertSentAt` is set only after at least one channel delivers. If both fail, the claim is released and a later tick retries. A claim older than `MERIDIAN_JOB_ALERT_CLAIM_MS` (default 2 minutes) can be taken over.

**Send idempotency:** a run stamps `payload.expoSendStartedAt` before Expo. A retry of that same run does not send again. Weekly drop outside the drop window releases that stamp, puts the run back to `pending` without burning an attempt, and waits 30 minutes so the week key is not consumed. Consensus reminders on a ritual scan are claimed per crew on the run and written to `meridian_job_deliveries` with the swipe rows.

Canonical task ledger: `Meridian-Mintlify/strategy/meridian-job-notification-platform-plan.mdx`. Relay-syntax laptop phases (not Mini notification execution): `relay/plans/meridian-job-notification-platform.md`.
