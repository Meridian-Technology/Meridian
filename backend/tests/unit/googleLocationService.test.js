...
describe('googleLocationService', () => {
  ...
  it('looks up a city bounding box and prefers locality results with bounds', async () => {
    ...
    const result = await createGoogleLocationAdapter(options).lookupCityBoundary('New York', {
      countryCode: 'US',
    });
    ...
    expect(result).toMatchObject({
      ...
      countryCode: 'US',
      bounds: { north: 40.92, east: -73.7, south: 40.48, west: -74.26 },
  ...
  it('falls back to viewport when Google omits official bounds', async () => {
    ...
    await expect(createGoogleLocationAdapter(options).lookupCityBoundary('Troy, NY'))
      .resolves.toMatchObject({
        formattedAddress: 'Troy, NY, USA',
        bounds: { north: 42.8, east: -73.6, south: 42.7, west: -73.8 },
      });
  });
  ...
  it('rejects a geocode result that is not a city boundary', async () => {
    ...
    await expect(createGoogleLocationAdapter(options).lookupCityBoundary('123 Main St'))
